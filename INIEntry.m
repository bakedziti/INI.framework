    ...
    if (self.line) {
        ...
        for (int i = 0; i < n; i++) {
            switch (state) {
                ...
                case ' ':
                    switch ([self.line characterAtIndex: i]) {
                ...
                case 'k':
                    switch ([self.line characterAtIndex: i]) {
                        case ' ':
                            ...
                            for (j = i; j < n && ([self.line characterAtIndex: j] == ' ' || [self.line characterAtIndex: j] == '\t'); j++) {}
                            if (j == n) {
                                // malformed entry, we were looking for = sign after the key and reached end of the line
                                state = '.';
                            } else {
                                if ([self.line characterAtIndex: j] == '=') {
                                    ...
                                    for (j = j + 1; j < n && ([self.line characterAtIndex: j] == ' ' || [self.line characterAtIndex: j] == '\t'); j++) {}
                                    i = j - 1;
                                    state = 'v';
...
- (NSString *) key {
    return [self.line substringWithRange: info.key];
}
...
- (NSString *) value {
    return [self.line substringWithRange: info.value];
}
...
- (NSString *) section {
    return [self.line substringWithRange: info.section];
}
